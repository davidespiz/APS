...
int main()
{
   ...
   cout << "Incasso massimo per giorno: " << IncassoGiornoMax(t, Data(16, 2, 2026)) << " euro" << endl << endl;
   ...
   try { t.RegistraSpettatore(&p1); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // RegistraSpettatore: "La persona è minorenne!"
   try { t.RegistraSpettatore(&minorenne); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // ProgrammaSpettacolo: "Data fuori stagione!"
   try { t.ProgrammaSpettacolo(&s1, Data(1, 1, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // ProgrammaSpettacolo: "Giorno già occupato!"
   try { t.ProgrammaSpettacolo(&s2, Data(17, 2, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // AcquistaBiglietto: "Persona non registrata!"
   try { t.AcquistaBiglietto(&nonRegistrata, Data(17, 2, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // AcquistaBiglietto: "Giorno senza spettacolo!"
   try { t.AcquistaBiglietto(&p1, Data(16, 2, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // AcquistaBiglietto: "La persona ha già un biglietto per questo giorno!"
   try { t.AcquistaBiglietto(&p1, Data(22, 2, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }

   // CancellaBiglietto: "Persona non registrata!"
   try { t.CancellaBiglietto(&nonRegistrata, Data(17, 2, 2026)); } catch (const invalid_argument& e) { cerr << e.what() << endl; }
}
